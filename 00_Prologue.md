...
In case the starting and arrival points of a path lie on the same meridian, the shortest of the two segments into which the two points divide the meridian is not only the ***shortest-length*** path, ut this is also the ***shortest-time*** path,  two points is the shortest path at in which  




<!--stackedit_data:
eyJoaXN0b3J5IjpbMTg4ODYzNTc3MV19
-->