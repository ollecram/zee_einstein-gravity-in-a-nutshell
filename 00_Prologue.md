...
Let now map the external and internal surfaces of the hemisphere into the external surface of a sphere, as we did in the previous exercise. In order to to exploit analogies with the Snell's law let us define some geometric objects on the sphere surface:
 ...
 - An infinite number of *maximum length circles* (*great circles* or *meridians*) contain the points $p$ and $P$ associated to a given circle $C_{[p,P]}$
...
There is nothing unique in this special case that is ue to working on the surface of a sphere fact here is a perfectly analogous situation in the flat geometry of Snell's law when $\theta_{a} = \theta_{w} = 0$. In that case the *shortest-time* path joining two points which are mirroring each other from opposite sides with respect to the air-water interface is the straight segment (*shortest-length* path) line joining them and orthogonal to the interface.  

In order derive the analogous of Snell's law on the sphere we should simply apply formulas of spherical geometry to compute the shortest length paths of the two velocity zones.   




<!--stackedit_data:
eyJoaXN0b3J5IjpbLTIwMzUyNzYwMjRdfQ==
-->