...
These two vectors do determine a plane. By choosing the origin of our coordinate axes $(\mathbf{i}, \mathbf{j}, \mathbf{k})$ in the field center,  the $\mathbf{k}$ axis parallel to $\mathbf{r} \times \dot{\mathbf{r}}$,  we can convince ourselves that the particle could not possibly deviate from the plane $z=0$, because the  trajectory deviate from that plane in the course of motion, that would violate one of our prior assumptions. 
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTE0MjQyNjA5NzBdfQ==
-->