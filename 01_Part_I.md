...
These two vectors do determine a plane. By choosing the origin of our coordinate axes $(\mathbf{i}, \mathbf{j}, \mathbf{k})$ in the field center and the $\mathbf{k}$ axis parallel to $\mathbf{r} \times \dot{\mathbf{r}}$, both the position vector $\mathbf{r}(t_0)$ and the velocity vector $\dot{\mathbf{r}}(t_0)$ are contained in the plane $z=0$. The force field component along $\mathbf{k}$ vanishes across the whole plane. because the  trajectory deviate from that plane in the course of motion, that would violate one of our prior assumptions. 
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTIwNzI3MzQ3OTEsLTU1NTk2OTMzMywxOD
kzNDM1MzU0XX0=
-->