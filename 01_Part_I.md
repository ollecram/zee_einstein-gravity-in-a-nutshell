...
By combining the above two statements one sees that at a given point in time (hence forgetting any knowledge of prior dynamical history) there are two vectodirections related to a particle in a central field: the the initialconditions a given problem must be determined by , the only directions that may all directions one can logically make use of in    
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbNTgxNDYyMDg2LDExNDU1NjM3NjVdfQ==
-->